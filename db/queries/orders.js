...
const getNewOrders = () => {
  ...
  SELECT o.id AS order_id, c.name AS customer_name, i.name AS item_name, mc.id AS category_id,
       oi.instructions, oi.quantity, o.start_time
  FROM orders o
...
const getCompletedOrders = () => {
  return db.query(`
  SELECT o.id AS order_id, c.name AS customer_name, i.name AS item_name, oi.quantity, oi.instructions, i.category_id, o.completion_time, o.picked_up
  FROM orders o
  JOIN customers c ON o.customer_id = c.id
  JOIN order_items oi ON o.id = oi.order_id
  JOIN menu_items i ON oi.item_id = i.id
